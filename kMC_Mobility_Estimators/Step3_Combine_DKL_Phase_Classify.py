...
##### Setting about input & output files: FNN for predicting phase of coupling #####
NN_archi = 'H4'  #Number of hidden layers of NN
main_folder = '/home/ciwang/DKL-ECG/Sign_Classification/'
feature_type = '_Inter' 
#sys.path.append('/Users/chuniwang/Downloads/Sign_Classification')
#sys.path.append(main_folder + filename_flag_train + feature_type)
if NN_archi=='H7':
  from subroutine_Binary_Classification import Net_H7
elif NN_archi=='H4':
  from subroutine_Binary_Classification import Net_H4
Model_name = '/CG_H4_BS500_LR00100_Best_Model.pth' #700K_H4_Inter
#Model_name = '/CG_H4_BS500_LR00050_Best_Model.pth' #700K_H7_Inter
ML_model_path = main_folder + '700K_' + NN_archi + feature_type + Model_name #700K
#700K_H4_Inter: '/CG_H4_BS500_LR00100_Best_Model.pth'
#700K_H7_Inter: '/CG_H4_BS500_LR00050_Best_Model.pth'
#555K_H4_Inter: '/CG_H4_BS500_LR00050_Best_Model.pth'
#555K_H7_Inter: '/CG_H7_BS500_LR00050_Best_Model.pth'


##### Setting about input & output files: DKL for predicting coupling in log scale #####
filename_flag = sys.argv[1]
use_previous_model_to_train = 'yes' #'yes'
file_folder = 'CG_NVT_' + sys.argv[2] + '_8Cells'  #type 700K or 555K
test_pred_filename = 'Coupling_' + sys.argv[2] + '_Config_' + filename_flag +'.txt'
output_dir = './' + file_folder + '/ECG_Results/'

#model_input = '/home/ciwang/DKL-ECG/700K_Coupling/Learn_Log/BS5K_LR002_LS15_Inter/'
model_input = '/home/ciwang/DKL-ECG/555K_Coupling/Learn_Log/BS5K_LR002_NN_Inter/'
...
##### Path of ML dataset #####
#big_dir = ['/home/ciwang/Electronic_Coupling/ML_Dataset/555K', ('/home/ciwang/Electronic_Coupling/ML_Dataset/555K/')]  #[traing, testing]
big_dir = ['/home/ciwang/Electronic_Coupling/ML_Dataset/555K', ('./' + file_folder + '/ML_Feature')]  #[traing, testing]
# these files should be contained in big_dir
#fn_data = ['Feature_Coupling_CG_Improve_25_Inter_Training_COM.npy', 'Label_Coupling_Sample_128000.npy',    #700K
fn_data = ['Feature_Coupling_CG_Improve_25_Inter_Training_COM.npy', 'Label_Coupling_Sample_92160.npy', #555K
           'Feature_Coupling_'+ sys.argv[2] +'_Config_' + filename_flag +'.npy', 'Label_Coupling_1_10240.npy']
...
def main():   
    ...
    ############### DKL for predicting coupling in log ###############
    ...
    ###### Machine Learning ######
    data_out = vdkl_experiment(n_train, big_dir, fn_data, param_job,'_ur', output_dir)
    V_linear = 10**(data_out[:,0])
    
    
    ############### FNN for predicting phase of coupling ###############
    V_phase = phase_classify()
    
    ############### Electronic coupling with phase ###############
    pred_V = V_linear * V_phase
    #print(pred_V[:5])
    #np.savetxt((output_dir+test_pred_filename), pred_V)
    V_all = np.vstack((pred_V, data_out[:,1])).T
    np.savetxt((output_dir+test_pred_filename), V_all) 


def phase_classify():
    device = "cpu" # cuda or cpu
    ###### Load training data set #####
    #X = np.load((big_dir[0] + '/' + fn_data[0]))
    X = np.load(('/home/ciwang/Electronic_Coupling/ML_Dataset/700K/' + fn_data[0]))
    train_xmean = np.mean(X,0)
    train_xstd = np.std(X,0)
    
    ###### Loading machine learning model #####
    D_feature = X.shape[1] #Check the dimenssion of input feature of Coulomb matrix 
    if NN_archi=='H7':
      model = Net_H7(D_feature)
    if NN_archi=='H4':
      model = Net_H4(D_feature)
    ...
    summary_accuracy = []
    test_data_temp = []
    test_data_temp.append(filename_flag)
    for filename_flag_test in test_data_temp:
      fname_xt = big_dir[1] + '/' + fn_data[2]
      fname_yt = big_dir[1] + '/' + fn_data[3]
      print("\nValidate model on %sK data set" %(filename_flag_test))
      X_test = np.load(fname_xt)
      ...
      #Y_test_value = np.load(fname_yt).flatten()
      ...
      print("Testing dataset:\nshape of x: {}".format(X_test.shape))
      #print('Number of posive/negative sign in validation dataset: %s/%s' %(Y_test.sum().item(),(Y_test.shape[0]-Y_test.sum()).item()))
      ...
      Y_predict_class = Y_predict.reshape(-1).detach().cpu().numpy().round()
      #Y_acc = (Y_predict_class == Y_test).mean()
      #summary_accuracy.append([int(filename_flag_test), Y_acc])
      #print("Model accuracy: {:.3f}".format(Y_acc))
      
      V_phase = np.zeros((no_data_test), dtype=float)
      V_phase = np.where((Y_predict_class==1),1.000000000,-1.000000000)
      return V_phase
...
def vdkl_trial(train_x, train_y, test_x, params, dn, f_ext, train_mean, output_dir):
    ...
    with torch.no_grad(), gpt_settings.fast_pred_var(),gpt_settings.max_cg_iterations(2500):
        ...
        data_out[:,0] = (pos_train_mean+train_mean).cpu()#Unit of meV in log scale
        data_out[:,1] = pos_pred_2.stddev.flatten().cpu() #Unit of meV in log scale
    return data_out
...
def vdkl_experiment(n_train, dn, fn_list, params, f_ext, output_dir):
    ...
    #################### Training Model ##################
    data_out = vdkl_trial(train_x, train_y, test_x, params, dn, f_ext, train_mean, output_dir)
