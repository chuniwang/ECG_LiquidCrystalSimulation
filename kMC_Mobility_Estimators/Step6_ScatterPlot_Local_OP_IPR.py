...
constant_onsite_energy = sys.argv[1] #'no'
H_type = sys.argv[2] #'Gaussian_sampling' 'ECG_mean'
...
cutoff_neighbor_dis = 50.0 #20.00 #13.00

figurename_scatter = []
for keyword in keyword_list:
  if (constant_onsite_energy == 'no'):
    if (H_type == 'ECG_mean'):
      figurename_scatter.append(('Scatter_OP_IPR_' + keyword + '_H_ECG_mean_Cut50.png'))
    if (H_type == 'Gaussian_sampling'):
      figurename_scatter.append(('Scatter_OP_IPR_' + keyword + '_H_Sampling_Cut50.png')) 
  if (constant_onsite_energy == 'yes'):
    if (H_type == 'ECG_mean'):
      figurename_scatter.append(('Scatter_OP_IPR_' + keyword + '_H_ECG_mean_Constant_SiteEnergy.png')) 
    if (H_type == 'Gaussian_sampling'):
      figurename_scatter.append(('Scatter_OP_IPR_' + keyword + '_H_Sampling_Constant_SiteEnergy.png')) 
...
def calculate_pipi(COM_1,COM_2,cross_vec_1,cross_vec_2):
      ########## Transfor input vectors into unit vectors ##########
      cross_vec_1 = cross_vec_1/LA.norm(cross_vec_1)
      cross_vec_2 = cross_vec_2/LA.norm(cross_vec_2)
      ...
      ########## Calculate dihedral angles between inter-rings ##########
      inner_product = np.sum(cross_vec_1*cross_vec_2)
      inner_product = inner_product/(cross_vec_1_length*cross_vec_2_length)
      torsion_angle = np.arccos(inner_product)*180/np.pi
      
      ########## Calculate level of inte-ring pi-pi stacking ##########
      pipi_level = ((np.dot(COM_vec,cross_vec_1))**2) * ((np.dot(COM_vec,cross_vec_2))**2) * ((np.dot(cross_vec_1,cross_vec_2))**2)
      pipi_level = pipi_level * 1 * np.exp(-(COM_dis-3.5))
      #print(pipi_level)
      return pipi_level 
      ...
      for sampling in range(sampling_start,sampling_end):    
        ...
        #for center in range(total_no_mol):    
          ...
          if (len(charge_index)<15):
            ...
            figure_data.append([Q_eigen_w.max().real,IPR[center].real,no_charge_mol])
...
for l in range(len(keyword_list)):
  ...
  plt.ylabel('Charge delocalization', fontsize=36)
  plt.ylim(1,10)
  plt.yticks(np.arange(1, 11, step=1), fontsize=28, fontname="Arial")
...
for i in range(no_sampling):
    ...
    #if not os.path.exists('./ECG_Results/Eigen_Value_' + keyword +'.npy'):
    if not os.path.exists('./ECG_Results/Eigen_Value_' + keyword + '_' + str(i+1) +'.npy'):
      print('./ECG_Results/Eigen_Value_%s.npy does not exist' %keyword)
    else:
      eigen_w = np.load(('./ECG_Results/Eigen_Value_' + keyword + '_' + str(i+1) +'.npy'))
      eigen_v = np.load(('./ECG_Results/Eigen_Vector_' + keyword + '_' + str(i+1) + '.npy'))
      #eigen_w = np.load(('./ECG_Results/Eigen_Value_' + keyword + '.npy'))
      #eigen_v = np.load(('./ECG_Results/Eigen_Vector_' + keyword + '.npy'))
      eigen_w = eigen_w.real
      eigen_v = eigen_v.real
    ...
    ######################## Move all the molecules in a cluster together  ########################
    #for t in range(6013,6014):
    #for t in range(6010,6024):
    for center in range(90):    
      ############ Find charge carrier molecules ############
      charge_index = np.argwhere(eigen_v[:,center]**2>0.0001)
      no_charge_mol = len(charge_index)
      traj_center = np.squeeze(traj_pbc[charge_index,:,:],axis=1)
      ...
      ### Cutoff based on separating distance ###
      ind = np.argwhere(dis[np.argsort(dis)]<cutoff_neighbor_dis)
      charge_neighbor_index = np.argsort(dis)[:ind[-1][0]].reshape(-1, 1)
      if (charge_neighbor_index.size==0): #Deal with empty list
        charge_neighbor_index = np.argsort(dis)[:15].reshape(-1, 1)
        #print(charge_neighbor_index)
        #print(center,dis[np.argsort(dis)][:2],dis[np.argsort(dis)[1]])
...
figure_data = np.array(figure_data)
#print(figure_data[:,0].min(),figure_data[:,0].max())
#print(figure_data[:,1].min(),figure_data[:,1].max())
print(figure_data.shape)
print(figure_data[:,2].max())
...
plt.figure(figsize = (10, 10), dpi=250)
##### Calculate the point density #####
#x, y = figure_data[:,0]*(figure_data[:,2]/figure_data[:,2].max()), figure_data[:,1]
#x, y = figure_data[:,2], figure_data[:,1]
x, y = figure_data[:,0], figure_data[:,1]
xy = np.vstack([x,y])
z = gaussian_kde(xy)(xy) 
...
plt.xlabel('Weighted Order parameter', fontsize=36)
plt.xticks(np.arange(0.0, 1.1, step=0.1), fontsize=28, fontname="Arial", rotation=45) #rotation=90
...
plt.xlabel('No. molecules', fontsize=36)
#plt.xticks(np.arange(0.0, 1.1, step=0.1), fontsize=28, fontname="Arial", rotation=45) #rotation=90
